...
-- Summary: 
Write a simple program that should display an options menu as can be seen below:
	P - Print numbers
	A - Add a number to the list
	M - Display the mean of the numbers on the list
	S - Display the smallest number
	L - Display the largest number
	Q - Quit
The program should only accept valid choices from the user, and the user
should be warned in case an illegal choice is made.
*/
	...
	do
	{
		...
		case 's':
			...
			else
			{
				for (unsigned int i{}; i < listNumbers.size(); i++)
				{
					if (listNumbers.at(i) < smallest)
					{
						smallest = listNumbers.at(i);
					}
				}
		...
		case 'l':
			...
			else
			{
				for (unsigned int i{}; i < listNumbers.size(); i++)
				{
					if (listNumbers.at(i) > largest)
					{
						largest = listNumbers.at(i);
					}
				}
